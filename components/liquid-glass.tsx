...
class Shader {
  ...
  private fragment: (uv: Point, mouse: Point) => TextureResult;
  ...
  constructor(
    ...
    options: {
      width: number;
      height: number;
      fragment: (uv: Point, mouse: Point) => TextureResult;
    }
  ) {
  ...
  private constrainPosition(x: number, y: number): Point {
  ...
  private updateShader(): void {
    const mouseProxy = new Proxy(this.mouse, {
      get: (target: Point, prop: keyof Point) => {
        this.mouseUsed = true;
        return target[prop];
      },
    });
...
const LiquidGlass: React.FC<LiquidGlassProps> = ({
  ...
  const fragment = useCallback((uv: Point, mouse: Point) => {
    const ix = uv.x - 0.5;
    const iy = uv.y - 0.5;
  ...
  useEffect(() => {
    ...
    shaderRef.current = new Shader(containerRef.current, {
      width,
      height,
      fragment,
    });
